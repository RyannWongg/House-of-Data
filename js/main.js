...
let series;                 // selection for all team groups
let legendItems;            // selection for legend pills
const visState = new Map(); // team -> visible?
...
const showAllBox = d3.select("#showAll");

// ======= Helpers (no data needed) =======
...
function updateShowAllCheckbox() {
  ...
  d3.select("#showAll").property("checked", allOn);
}
...
// Map of common names (Title Case) and historical variants → colors
...
// After we know the team list, we’ll build a distinct fallback palette
let colorFn; // assigned after data load

// ======= Main: load data then init =======
d3.csv(CSV_PATH).then(raw => {
  ...
  clearBtn.on("click", () => {
    ...
    d3.select("#legend").selectAll(".legend-item").classed("off", false);
  });
  ...
  showAllBox.on("change", (e) => {
    ...
    applyVisibility();
    d3.select("#legend").selectAll(".legend-item").classed("off", !show);
