export function renderComparison(sel) {    
    ...
    if (comparisonTeamSelect) {
        comparisonTeamSelect.addEventListener('change', (e) => {
            ...
            if (currentComparisonTeam) {
                ...
                if (chartSvg.node()) svg.selectAll('*').remove();
            }
        });
    }
    ...
    async function loadComparison(team) {
        try {
            ...
            createComparisonChart(chartSvg, data2000, data2025);
    ...
    // Comparison function 
    function createComparisonChart(svg, data2000, data2025) {
        if (!svg.node()) return;
